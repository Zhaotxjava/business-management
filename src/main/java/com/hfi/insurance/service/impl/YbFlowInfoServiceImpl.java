...
//        Page<YbFlowInfo> page = new Page<>(req.getPageNum(), req.getPageSize());
        ...
        if(queryWrapper == null){
            return new GetSignedRecordBatchRes();
        }
...
//            queryWrapper.likeRight("subject", req.getTemplateId());
        ...
        if (StringUtils.isNotEmpty(req.getEndInitiateTime())) {
            //<=
            queryWrapper.le("initiator_time", req.getEndInitiateTime() + " 23:59:59");
        }
        BatchQueryTypeEnum type = BatchQueryTypeEnum.getType(req.getQueryType());
        if (type == BatchQueryTypeEnum.BLANK || !req.getNumbers().isEmpty()) {
            switch (type) {
                case SINGLE_NUMBER:
                    if (StringUtils.isNotBlank(req.getNumbers().get(0))) {
                ...
                case SINGLE_NAME:
                    ...
                    if (!tempNumber.isEmpty()) {
                        ...
                        log.info(" SINGLE_NAME 未查询到符合条件的机构");
                        return null;
                    }
                    break;
                case NUMBERS:
